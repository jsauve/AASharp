...
namespace AASharpTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            AASDate dateSunCalc(2007, 9, 21, true);
            ...
            AASDate dateMoonCalc(2007, 9, 21, true);
            ...
            for (int YYYY=-4712; YYYY<5000; YYYY++) //Change the end YYYY value if you would like to test a longer range
            {
                ...
                {
                    bool bLeap = AASDate.IsLeap(YYYY, (YYYY >= 1582));
                    for (int DDDD=1; DDDD<=AASDate.DaysInMonth(MMMM, bLeap); DDDD++)
                    {
                        bool bGregorian = AASDate.AfterPapalReform(YYYY, MMMM, DDDD);
                        AASDate date(YYYY, MMMM, DDDD, 12, 0, 0, bGregorian);
                        ...
                        if (bGregorian)
                        {
                            AASCalendarDate GregorianDate(AASDate.JulianToGregorian(YYYY, MMMM, DDDD));
  ...
  AASEllipticalPlanetaryDetails VenusDetails = AASElliptical.Calculate(2448976.5, AASElliptical.VENUS);

  AASEllipticalPlanetaryDetails SunDetails = AASElliptical.Calculate(2453149.5, AASElliptical.SUN);
  ...
  AASParabolicObjectElements elements4;
  ...
  AASEllipticalObjectElements elements5;
  ...
  int MM = 8;
  int DD = 10;
  AASDate CalcDate(YYYY, MM, DD, true);
  JD2 = CalcDate.Julian;  
  ...
  CAATopocentricEclipticDetails TopocentricDetails = AASParallax.Ecliptic2Topocentric(AASCoordinateTransformation.DMSToDegrees(181, 46, 22.5), AASCoordinateTransformation.DMSToDegrees(2, 17, 26.2), 
  ...
  AASCalendarDate JulianDate = CAAMoslemCalendar::MoslemToJulian(1421, 1, 1);
  ...
  AASCalendarDate MoslemDate = CAAMoslemCalendar::JulianToMoslem(JulianDate2.Year, JulianDate2.Month, JulianDate2.Day);
  bLeap = CAAMoslemCalendar::IsLeap(1421);

  MoslemDate = CAAMoslemCalendar::JulianToMoslem(2006, 12, 31);
  AASCalendarDate OriginalMoslemDate = CAAMoslemCalendar::MoslemToJulian(MoslemDate.Year, MoslemDate.Month, MoslemDate.Day);
  ...
  AASCalendarDate MoslemDate2 = CAAMoslemCalendar::JulianToMoslem(JulianDate3.Year, JulianDate3.Month, JulianDate3.Day);
  AASCalendarDate JulianDate4 = CAAMoslemCalendar::MoslemToJulian(MoslemDate2.Year, MoslemDate2.Month, MoslemDate2.Day);
  AASCalendarDate GregorianDate2 = AASDate.JulianToGregorian(JulianDate4.Year, JulianDate4.Month, JulianDate4.Day);

  AASCalendarDate JewishDate = CAAJewishCalendar::DateOfPesach(1990);
  ...
  return 0;
        ...
        void LunarRaDec(double JD, ref double RA, ref double Dec)
        {
